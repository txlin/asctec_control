...
std::string q_name = "/asctec_so3_control";
std::string q_frame = "/asctec_dummy"; 
...
void timerCallback(const ros::TimerEvent& event) 
{
  ...
  transform.setRotation(q);
	tf::quaternionTFToMsg(q,odom.transform.rotation);
  br->sendTransform(tf::StampedTransform(transform, ros::Time::now(), world, q_frame));
...
int main(int argc, char **argv)
{
	...
	odom_pub = n.advertise<geometry_msgs::TransformStamped>(q_frame, 10); 
	ros::Subscriber odom_sub = n.subscribe(q_name+"/dummy", 10, odomCallback);
	ros::Subscriber pos_sub, ang_sub;
	if(track) pos_sub = n.subscribe(q_name+"/position_cmd", 10, cmdCallback);
	if(track) ang_sub = n.subscribe(q_name+"/si_command", 10, siCallback);
  ...
  ros::Timer timer = n.createTimer(ros::Duration(1/rate), timerCallback);
	...
	ROS_INFO("Broadcasting dummy frame in %s...", q_frame.c_str());
