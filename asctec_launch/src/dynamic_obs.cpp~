...
bool avoid = false, adapt = false, start2 = false;
...
void sendFinalTrajectory1(float time, float x, float y, float z, float yaw, bool reset) 
{
	asctec_msgs::WaypointCmd cmd;
	cmd.position.x = x;
	cmd.position.y = y;
...
void sendTrajectory1(float time, float x, float y, float z, float vel_x, float vel_y, float vel_z, float yaw, bool reset) 
{
	asctec_msgs::WaypointCmd cmd;
	cmd.position.x = x;
	cmd.position.y = y;
...
void odomCallback1(const nav_msgs::Odometry::ConstPtr& msg)
{
	...
	if (isDone1 && hover1){
		...
		// go to the initial hovering position
		flying1 = true;
		move1 = false;
		...
		toGoal = true;
	}else if(isStart1 && toGoal && adapt && !avoid){
		// move to the final goal while not avoiding (decided on matlab side)
		flying1 = true;
		adapt = false;
		...
		tTravel = limit(tTravel, 10, 1);
		sendFinalTrajectory1(tTravel,  goal_x, goal_y, goal_z, 0, 1);	
	}else if(toGoal && !adapt && avoid){
		// when avoid signal comes from the matlab side, move to avoid position
		...
		sendTrajectory1(1.35, odom1_.pose.pose.position.x-0.25, odom1_.pose.pose.position.y+0.4, odom1_.pose.pose.position.z, 0.3*avoid_dir_x, 0.3*avoid_dir_y, 0, 0, true);
		//sendTrajectory1(1.35, odom1_.pose.pose.position.x-0.25, odom1_.pose.pose.position.y+0.4, odom1_.pose.pose.position.z, 0, 0.25, 0, 0, true);
		...
		tTravel = limit(tTravel, 10, 1);
		sendTrajectory1(tTravel,  avoid_x, avoid_y, avoid_z, avoid_vel_x, avoid_vel_y, avoid_vel_z, 0, false);	
		...
		tTravel = limit(tTravel, 10, 1);
		sendTrajectory1(tTravel,  goal_x, goal_y, goal_z, 0, 0, 0, 0, false);			
	}else if(isDone1 && land1){
		sendTrajectory1(8, 0, 0, 1, 0, 0, 0, 0, true);
...
void odomCallback2(const nav_msgs::Odometry::ConstPtr& msg)
{
	...
	if (isDone2 && hover2){
		...
		if (toStart){
			...
			toFinal = true;
		}else if(start2 && toFinal){
			ROS_INFO("quad2 moving to final position");			
			toFinal = false;
			// for collision wait 2 sec, for passing wait 0.75 sec
			sendTrajectory2(2, odom2_.pose.pose.position.x, odom2_.pose.pose.position.y, odom2_.pose.pose.position.z, 0);
...
int main(int argc, char** argv) {
	...
	ros::Subscriber adapt_sub1 = nh.subscribe(group_ns1 + "/adapt", 10, adaptCallback);
	ros::Subscriber avoid_sub1 = nh.subscribe(group_ns1 + "/avoid", 10, avoidCallback);
	ros::Subscriber start_sub2 = nh.subscribe(group_ns2 + "/start", 10, startCallback);
	...
	new_wpt_pub = nh.advertise<nav_msgs::Odometry>(group_ns1 + "/new_wpt", 10);
